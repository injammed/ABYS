...
export function CurrencyObjectViewer({item}:{item:Item}) {
  ...
  useEffect(()=>{
    ...
    Promise.all([import("three"),import("three/addons/controls/OrbitControls.js"),import("three/addons/environments/RoomEnvironment.js"),import("@/lib/currency-sculpture")]).then(([T,{OrbitControls},{RoomEnvironment},{buildCurrencySculpture,disposeCurrencySculpture}])=>{
      ...
      renderer.setPixelRatio(Math.min(devicePixelRatio,2));renderer.setClearColor(0x090e17);parent.appendChild(renderer.domElement);
      ...
      const controls=new OrbitControls(camera,canvas);controls.enablePan=false;controls.enableDamping=false;controls.minDistance=1.6;controls.maxDistance=7;
      const model=buildCurrencySculpture(item);scene.add(model);
      ...
      scene.add(new T.HemisphereLight(0xd9e8ff,0x51402c,2));const light=new T.DirectionalLight(0xffe9c4,3);light.position.set(4,5,3);scene.add(light);
      ...
      cleanup=()=>{resize.disconnect();controls.removeEventListener("change",render);controls.dispose();canvas.removeEventListener("keydown",keys);disposeCurrencySculpture(model);env.dispose();renderer.dispose();canvas.remove();};
  ...
  async function download(){
    ...
    try{
      ...
      const model=buildCurrencySculpture(item);cleanup=()=>disposeCurrencySculpture(model);
