...
export function PrimaryNavigation({ mode }: { mode?: "feed" | "museum" | "shop" | "argus" }) {
  return (
    <nav className="primary-navigation" aria-label="Upload, scroll, vote, shop, Argus" data-navigation-contract="upload-scroll-vote-shop-argus-v3">
      ...
      <div className="primary-action-rail">
        ...
        <Link href="/argus/" className="primary-action" aria-current={mode === "argus" ? "page" : undefined}>
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 12s4-7 10-7 10 7 10 7-4 7-10 7S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/></svg><span>ARGUS</span>
        </Link>
      </div>
    </nav>
  );
