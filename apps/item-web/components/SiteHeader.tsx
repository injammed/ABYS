...
export function SiteHeader({ mode }: { mode?: "feed" | "museum" | "argus" }) {
  return (
    <header className="site-header gallery-header">
      <Link href="/" className="gallery-wordmark" aria-label="AETIMM — home of Argus">
        <span className="gallery-emblem" aria-hidden="true">◇</span>
        <span>AETIMM<small>HOME OF ARGUS</small></span>
      </Link>
      <nav className="primary-mode-switch" aria-label="Choose primary experience">
        <Link href="/argus/" className={`primary-mode-link${mode === "argus" ? " active" : ""}`} aria-current={mode === "argus" ? "page" : undefined}>Argus</Link>
