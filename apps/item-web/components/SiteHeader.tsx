...
export function SiteHeader({ mode }: { mode?: "feed" | "museum" | "apyoc" | "library" }) {
  return (
    <header className="site-header gallery-header">
      <Link href="/" className="gallery-wordmark" aria-label="AETIMM — Library of Things">
        <span className="gallery-emblem" aria-hidden="true">◇</span>
        <span>AETIMM<small>LIBRARY OF THINGS</small></span>
      </Link>
      <nav className="primary-mode-switch" aria-label="Choose primary experience">
        <Link href="/" className={`primary-mode-link${mode === "library" ? " active" : ""}`} aria-current={mode === "library" ? "page" : undefined}>Library</Link>
        <Link href="/slop-trough/#field" className={`primary-mode-link primary-mode-feed${mode === "feed" ? " active" : ""}`} aria-current={mode === "feed" ? "page" : undefined}>Trough</Link>
