...
for (const [label, pattern, source] of [
  ...
  ["tenfold dark derangement law", /const DARK_DERANGEMENT_MULTIPLIER = 10;/, lexicon],
  ...
  ["dark mode has three zero-clock fracture planes", /\.fractureNorth[\s\S]*\.fractureSouth[\s\S]*\.fractureDepth/, lexiconStyles],
  ["fracture planes remain static optical copies", /className=\{`\$\{styles\.fracture\}[\s\S]*fractureNorth[\s\S]*fractureSouth[\s\S]*fractureDepth/, lexicon],
